...
=========================
    ...
    document.querySelector('[name=nombre]').value; para agarrar elementos con un atributo en particular con css- name(nombre del atributo)

    los botones radio se agrupna por 'name'. con el mismo name solo se puede sellecionar uno.

    document.querySelector('#carta-a-santa).comportamiento.value --- 
    carta a santa es el id del form. 
    Comportamiento es el name de los radio button
    .value nos da el button seleccionado

    document.formulario funciona pq el formulario tiene el "name"=formulario. En realidad seria: document.nameformulario
...
================================
  ...
  // (/^) eso significa que se arranca a chequear la informacion desde el inicio del texto
  // [A-z] eso significa que los caracteres van de la A a la z
  // + significa que es un caracter o mas
  // * siginica que puede venir 0 o mas numeros. es decir puede haber numeros o no y va a dar true
  // {1} poniendo entre llaves especifico la cantidad de numeros que pueden venir
  // $ el signo pesos marca el final. /^abc[0-9]{2,3}$ seria abc, entre dos y tres numeros, ahi termina la condicion.
  // /^abc[a-z]+/i agregando la i al final ponemos la condicion de que no distinga entre mayusculas y minusculas.
